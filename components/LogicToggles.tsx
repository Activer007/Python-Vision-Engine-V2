...
import { ToggleLeft, Lightbulb, CheckCircle2, XCircle, Filter } from 'lucide-react';
...
export const LogicToggles: React.FC<Props> = ({ setConsole }) => {
  ...
  useEffect(() => {
    setConsole(
      `Level 4: 逻辑开关。<b>Bool (布尔值)</b> 只有 <span class='text-pve-green'>True</span> 和 <span class='text-pve-red'>False</span>。筛选就像过筛子，条件为真的留下，假的漏掉。`
    );
  }, []);

  useEffect(() => {
    setConsole(
      `🤖 逻辑运算：<span class="font-mono">score <b class="text-pve-amber">${operator}</b> ${threshold}</span>。<br/>结果为 <span class="text-pve-green font-bold">True</span> 的行亮起。共选中 <span class="text-white font-bold">${trueCount}</span> 行。`
    );
    ...
    <div className="flex flex-col h-full space-y-6 p-4">
        ...
        {LOGIC_DATA.map(student => {
          ...
          return (
            ...
            >
              {/* Light Status */}
              <div
                className={`absolute -top-3 p-2 rounded-full border-2 bg-slate-900 transition-colors duration-300 ${isActive ? 'border-pve-amber text-pve-amber shadow-lg' : 'border-slate-700 text-slate-700'}`}
              >
                {isActive ? (
              ...
              {/* Boolean Badge */}
              <div
                className={`mt-6 px-3 py-1 rounded text-xs font-mono font-bold uppercase flex items-center gap-1 ${isActive ? 'bg-pve-green/20 text-pve-green' : 'bg-pve-red/20 text-pve-red'}`}
              >
              ...
              <div
                className={`absolute bottom-0 w-0.5 h-full -z-10 ${isActive ? 'bg-gradient-to-t from-pve-amber to-transparent opacity-50' : 'hidden'}`}
              ></div>
            </div>
          );
        })}
      ...
      <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 flex justify-between items-center">
        <div className="text-sm text-slate-400 flex items-center gap-2">
          <Filter size={16} /> Pandas Filtering Result:
        </div>
