...
export const BracketLens: React.FC<Props> = ({ setConsole }) => {
  ...
  useEffect(() => {
    setConsole(
      'Level 1: 括号透视镜。请将鼠标悬停在代码中的括号上，听听【翻译官】是如何【实时解说】当前代码逻辑的。'
    );
  ...
  const tokens = useMemo(() => {
    ...
    const flushBuffer = () => {
      if (buffer) {
        if (buffer.trim()) {
          result.push({
            id: `content-${result.length}-${idCounter++}`,
            char: '',
            depth: currentDepth,
            partnerId: null,
            type: 'content',
  ...
  const getBracketBg = (char: string, isActive: boolean) => {
    ...
    if (char === '(' || char === ')')
      return 'bg-pve-amber/20 shadow-[0_0_20px_rgba(245,158,11,0.6)]';
    if (char === '[' || char === ']')
      return 'bg-pve-green/20 shadow-[0_0_20px_rgba(34,197,94,0.6)]';
    if (char === '{' || char === '}')
      return 'bg-pve-purple/20 shadow-[0_0_20px_rgba(168,85,247,0.6)]';
    return '';
  };
  ...
  const interpretToken = (token: BracketToken) => {
    ...
    const subjectName =
      prevText && prevText.match(/[\w_]+$/) ? prevText.match(/[\w_]+$/)?.[0] : '匿名对象';
    ...
    if (token.char === '(' || token.char === ')') {
      const isFuncCall =
        prevToken && prevToken.type === 'content' && prevToken.content?.trim().match(/[\w_]$/);
    ...
    // === SQUARE [] ===
      ...
      const isIndexing =
        prevToken &&
        ((prevToken.type === 'content' &&
          (prevToken.content?.trim().match(/[\w_]$/) ||
            prevToken.content?.trim().endsWith('"') ||
            prevToken.content?.trim().endsWith("'"))) ||
  ...
  const isHighlighted = (token: BracketToken) => {
    ...
    if (hoveredToken && hoveredToken.partnerId) {
      const start = tokens.findIndex(
        t => t.id === (isOpening(hoveredToken) ? hoveredToken.id : hoveredToken.partnerId)
      );
      const end = tokens.findIndex(
        t => t.id === (isOpening(hoveredToken) ? hoveredToken.partnerId : hoveredToken.id)
      );
    ...
    <div className="flex flex-col h-full space-y-4 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-slate-800 p-3 rounded-lg border border-slate-700 flex items-center gap-3">
          <div className="bg-pve-amber/10 p-2 rounded text-pve-amber">
            <PlayCircle size={20} />
          </div>
          <div>
            <div className="text-pve-amber font-bold text-sm">() 动作/组合</div>
            <div className="text-slate-500 text-xs">执行函数 · 元组打包</div>
          </div>
        </div>
        <div className="bg-slate-800 p-3 rounded-lg border border-slate-700 flex items-center gap-3">
          <div className="bg-pve-green/10 p-2 rounded text-pve-green">
            <LayoutGrid size={20} />
          </div>
          <div>
            <div className="text-pve-green font-bold text-sm">[] 定位/存储</div>
            <div className="text-slate-500 text-xs">索引取值 · 列表容器</div>
          </div>
        </div>
        <div className="bg-slate-800 p-3 rounded-lg border border-slate-700 flex items-center gap-3">
          <div className="bg-pve-purple/10 p-2 rounded text-pve-purple">
            <Map size={20} />
          </div>
          <div>
            <div className="text-pve-purple font-bold text-sm">{'{}'} 映射/唯一</div>
            <div className="text-slate-500 text-xs">字典查找 · 无序集合</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-slate-900 rounded-lg p-4 md:p-8 flex flex-col items-center justify-center border border-slate-700 relative overflow-hidden shadow-inner min-h-[200px]">
        <div className="relative text-2xl md:text-4xl font-mono tracking-wider flex flex-wrap justify-center items-center gap-y-4 leading-relaxed max-w-5xl z-10 select-none">
          {tokens.map(token => {
            ...
            const colorClass =
              token.type === 'bracket' ? getBracketColor(token.char) : 'text-slate-400';
            const bgClass =
              token.type === 'bracket'
                ? getBracketBg(token.char, active)
                : active
                  ? 'text-slate-200'
                  : '';
            ...
            return (
              ...
              >
                ...
                {token.type === 'bracket' && active && (
                  <span
                    className="absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] bg-slate-700 text-slate-300 px-1 rounded opacity-0 animate-fade-in-up"
                    style={{ opacity: 1 }}
                  >
                    L{token.depth}
                  </span>
        ...
        {analysis ? (
          <div className="flex flex-row h-full animate-fade-in-up">
            ...
            <div className="p-4 flex-1 flex flex-col md:flex-row gap-4 items-start md:items-center">
              <div
                className={`p-3 rounded-full bg-slate-900 border border-slate-700 ${analysis.color} shrink-0`}
              >
                {analysis.icon}
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs bg-slate-800 px-2 py-0.5 rounded text-slate-400 border border-slate-700 font-mono">
                    {analysis.syntax}
                  </span>
                </div>
                ...
                <div className="mt-2 text-xs text-slate-500 flex items-center gap-2">
                  <Quote size={12} /> 形象比喻：
                  <span className="text-slate-400 font-bold">{analysis.metaphor}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-slate-800 p-3 rounded-xl border border-slate-700 flex items-center gap-3">
        <label className="text-xs uppercase text-slate-500 font-bold whitespace-nowrap">
          代码 Input:
        </label>
        <input
          type="text"
