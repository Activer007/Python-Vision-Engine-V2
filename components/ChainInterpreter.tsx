...
import { GitCommit, XCircle, Edit3, Filter, Plus, ArrowUpDown, Calculator } from 'lucide-react';
...
export const ChainInterpreter: React.FC<Props> = ({ setConsole }) => {
  ...
  const getSingleValue = () => {
    ...
    const validScores = CLEANING_DATA.filter(r => r.isValid && r.score !== null).map(
      r => r.score as number
    );
  ...
  const getCurrentCode = () => {
    if (scenario === 'CLEANING') {
      switch (step) {
        case 0:
          return <span className="text-slate-500">data</span>;
        case 1:
          return (
            <span>
              <span className="text-slate-500">data</span>
              <span className="text-pve-red font-bold">.dropna()</span>
            </span>
          );
        case 2:
          return (
            <span>
              <span className="text-slate-500">data.dropna()</span>
              <span className="text-pve-purple font-bold">.rename(cols=...)</span>
            </span>
          );
        case 3:
          return (
            <span>
              <span className="text-slate-500">data...rename()</span>
              <span className="text-pve-blue font-bold">.mean()</span>
            </span>
          );
      }
    } else {
      switch (step) {
        case 0:
          return <span className="text-slate-500">df</span>;
        case 1:
          return (
            <span>
              <span className="text-slate-500">df</span>
              <span className="text-pve-amber font-bold">.query(&quot;score &gt;= 80&quot;)</span>
            </span>
          );
        case 2:
          return (
            <span>
              <span className="text-slate-500">df...query()</span>
              <span className="text-pve-green font-bold">.assign(grade=&quot;A&quot;)</span>
            </span>
          );
        case 3:
          return (
            <span>
              <span className="text-slate-500">df...assign()</span>
              <span className="text-pink-500 font-bold">.sort_values(&quot;score&quot;)</span>
            </span>
          );
      }
    }
  ...
  const getExplanation = () => {
    if (scenario === 'CLEANING') {
      switch (step) {
        case 0:
          return "🤖 准备就绪：加载原始数据。注意看，有些地方是空的 (<span class='text-pve-red'>NaN</span>)，那是我们要处理的脏数据。";
        case 1:
          return "🧹 <b>.dropna()</b>：启动吸尘器！把所有包含空值 (NaN) 的行<span class='text-pve-red font-bold'>直接丢弃</span>，只保留完整的数据。";
        case 2:
          return "🏷️ <b>.rename()</b>：贴换标签。把原来的列名 &quot;score&quot; 撕下来，换成更正式的 &quot;<span class='text-pve-purple font-bold'>Final_Grade</span>&quot;。";
        case 3:
          return "📊 <b>.mean()</b>：压缩汇总。把整张表格的数据&ldquo;捏&rdquo;在一起，算出一个<span class='text-pve-blue font-bold'>平均值</span>。";
      }
    } else {
      switch (step) {
        case 0:
          return '🤖 准备就绪：加载 DataFrame &quot;df&quot;。我们来看看谁是优等生。';
        case 1:
          return "🔍 <b>.query()</b>：设置安检门。只有分数 <span class='text-pve-amber font-bold'>&ge; 80</span> 的数据才能通过，其他的都被拦在门外。";
        case 2:
          return "➕ <b>.assign()</b>：发放奖励。给通过筛选的所有人发一个新的标签 &quot;<span class='text-pve-green font-bold'>grade = A</span>&quot;。";
        case 3:
          return "📉 <b>.sort_values()</b>：重新排队。按照分数<span class='text-pink-500 font-bold'>从高到低</span> (Descending) 的顺序重新排列队伍。";
      }
    }
    return '';
  };
          ...
          <div className="animate-scale-in flex flex-col items-center gap-4">
            <div className="w-48 h-48 rounded-full bg-slate-800 border-4 border-pve-blue flex items-center justify-center shadow-[0_0_50px_rgba(59,130,246,0.3)]">
              <div className="flex flex-col items-center">
                ...
                <div className="text-5xl font-mono font-bold text-white">{getSingleValue()}</div>
              </div>
            </div>
          ...
          <div className="w-full max-w-3xl transition-all duration-500 transform">
            <div className="bg-slate-800 rounded-lg overflow-hidden border border-slate-600 shadow-2xl relative">
              ...
              >
                ...
                <div>Name</div>
                <div
                  className={`transition-colors duration-500 flex items-center gap-2 ${scenario === 'CLEANING' && step >= 2 ? 'text-pve-purple font-bold' : ''}`}
                >
                  {scenario === 'CLEANING' && step >= 2 ? 'Final_Grade' : 'Score'}
                  {scenario === 'CLEANING' && step >= 2 && <Edit3 size={12} />}
                  {scenario === 'ANALYTICS' && step >= 3 && (
                    <ArrowUpDown size={12} className="text-pink-500" />
                  )}
                </div>
                  ...
                  >
                    ...
                    <div className="font-mono flex items-center relative">
                      {row.score === null ? (
                        ...
                        <span
                          className={`${scenario === 'ANALYTICS' && row.score && row.score >= 80 ? 'text-pve-green' : 'text-slate-300'}`}
                        >
                          {row.score}
                        </span>
                      )}
