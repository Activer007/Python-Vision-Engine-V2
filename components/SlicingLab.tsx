...
import { Scissors, MoveHorizontal, ArrowRightLeft, MousePointerClick } from 'lucide-react';
...
export const SlicingLab: React.FC<Props> = ({ setConsole }) => {
  ...
  const effStop = stop === '' ? (step > 0 ? SAMPLE_TEXT.length : -SAMPLE_TEXT.length - 1) : Number(stop);
  const effStep = step === 0 ? 1 : step; // Prevent infinite loop if 0

  useEffect(() => {
    setConsole("Level 7: 切片实验室 (Slicing Lab)。掌握 Python 独特的 <span class='text-pve-amber font-bold'>[start : stop : step]</span> 语法。注意观察上方(正)和下方(负)的索引编号。");
  }, []);

  useEffect(() => {
     const result = calculateSlice().join("");
     setConsole(`🤖 切片结果：<span class="text-pve-amber font-bold">"${result}"</span>。<br/>规则：从索引 ${effStart} 开始，切到 ${effStop} 之前 (不含)，步长为 ${effStep}。`);
  }, [start, stop, step]);
  ...
  const calculateSlice = () => {
    ...
    let s = effStart < 0 ? effStart + len : effStart;
    let e = effStop < 0 ? effStop + len : effStop;
      ...
      <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             ...
             <div className="bg-slate-900 p-3 rounded border border-slate-700 flex flex-col gap-2">
                ...
                <input 
                    type="range" min={-SAMPLE_TEXT.length} max={SAMPLE_TEXT.length} 
                    value={start === '' ? 0 : start} 
             ...
             <div className="bg-slate-900 p-3 rounded border border-slate-700 flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={() => setStep(-1)} className="px-2 py-1 bg-slate-700 text-xs rounded hover:bg-slate-600">Reverse</button>
                    <input 
      ...
      <div className="flex-1 bg-slate-900 rounded-xl border border-slate-700 relative overflow-hidden flex flex-col items-center justify-center p-2 md:p-8">
        ...
        <div className="flex gap-1 md:gap-2 overflow-x-auto max-w-full pb-8 pt-8 px-4">
            {SAMPLE_TEXT.split('').map((char, idx) => {
                ...
                const isStart = (idx === effStart) || (effStart < 0 && idx === effStart + SAMPLE_TEXT.length);
                // Note: Stop visual is tricky because it's exclusive, usually between characters
                
                return (
                    <div key={idx} className="relative flex flex-col items-center group">
                        
                        {/* Positive Index (Top) */}
                        <div className={`text-xs font-mono mb-2 transition-colors ${active || hoverIndex === idx ? 'text-white font-bold' : 'text-slate-600'}`}>
                            {idx}
                        </div>
                        ...
                        <div className={`text-xs font-mono mt-2 transition-colors ${active || hoverIndex === idx ? 'text-pve-red font-bold' : 'text-slate-700'}`}>
        ...
        <div className="absolute bottom-8 flex flex-col items-center text-slate-500 gap-2 opacity-50">
             <div className="flex items-center gap-4 text-xs uppercase tracking-widest">
                <div className="flex items-center gap-1"><ArrowRightLeft size={14}/> 正负索引对照</div>
                <div className="flex items-center gap-1"><MousePointerClick size={14}/> 悬停查看</div>
             </div>
